...
def extract_frozen_tag(repo_node: Any) -> str | None:
    """
    Extract '# frozen: <tag>' from the comment attached to 'rev'.
    """
    ...
    # End-of-line comment is index 2
...
async def resolve_tag(repo: str, tag: str) -> str | None:
    """
...
async def validate_repo(repo_node: Any) -> list[str]:
    ...
    tag_sha = await resolve_tag(repo, tag)

    if tag_sha is None:
        errors.append(f"{repo}: tag {tag!r} not found")
        return errors

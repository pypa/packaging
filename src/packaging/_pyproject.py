...
class PyProjectReader(ErrorCollector):
    ...
    def ensure_people(
        ...
        """
        Ensure that a value is a list of tables with optional "name" and
        "email" keys.
        """
        if not isinstance(val, list):
            msg = (
    ...
    def get_license(
        ...
        """
        Get the license field from the project table. Handles PEP 639 style
        license too.
        ...
        if isinstance(val, dict):
            ...
            msg = (
                "Field {key} has an invalid type, expecting"
    ...
    def get_readme(
        ...
        if isinstance(readme, str):
            ...
            msg = (
                "Field {key} has an invalid type, expecting either"
                " a string or table of strings"
            )
    ...
    def get_dependencies(self, project: ProjectTable) -> list[Requirement]:
        ...
        try:
            ...
            msg = (
                "Field {key} contains an invalid PEP 508"
                " requirement string {req!r} ({error!r})"
            )
    ...
    def get_optional_dependencies(
        ...
        if not isinstance(val, dict):
            msg = (
                "Field {key} has an invalid type, expecting"
                " a table of PEP 508 requirement strings"
            )
            self.config_error(
    ...
    def get_entrypoints(self, project: ProjectTable) -> dict[str, dict[str, str]]:
        ...
        if not isinstance(val, dict):
            msg = (
                ...
                " expecting a table of entrypoint sections"
            )
    ...
    def _get_files_from_globs(
        ...
        for glob in globs:
            if glob.startswith(("..", "/")):
                msg = (
                    "{glob!r} is an invalid {key} glob: the pattern must match "
            ...
            if not files:
                msg = (
                    "Every pattern in {key} must match at least one file:"
                    " {glob!r} did not match any"
                )
                self.config_error(msg, key="project.license-files", glob=glob)
                break
            for f in files:
